...
namespace Aerolinea.Business
{
    public class RutasCRUD : IRutasCRUD
    {
        ...
        {
            return rutasList.Where(x => x.Origen == origen && x.Destino == destino).FirstOrDefault();
        }

        public List<Ruta> BuscarRutasOrigen(string origen)
        {
            return rutasList.Where(x => x.Origen == origen).ToList();
        }
        ...
        //public List<Ruta> BuscarRutasOrigen(string origen)
        //{
        //    return rutas.Get(x => x.Origen == origen);
        //}
    }
}
